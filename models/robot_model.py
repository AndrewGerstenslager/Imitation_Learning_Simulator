...
from keras.layers import Input, Dense, Conv2D, Flatten, ReLU, Concatenate, Softmax
...
def get_model():
    ...
    x_image = Conv2D(filters=8, kernel_size=(4, 4), activation="relu", padding="same")(
        image_input
    )
    x_image = Conv2D(filters=16, kernel_size=(4, 4), activation="relu", padding="same")(
        x_image
    )
